#ifndef EUQ_TURB_SA_H
#define EUQ_TURB_SA_H
...
class EUQ_Turb_SA :virtual public JoeWithModels, virtual public RansTurbSA
{
public:   // constructors

	EUQ_Turb_SA(char *name) : JoeWithModels(name) , UgpWithCvCompFlow(name)
	{
		if (mpi_rank == 0)
		{
			cout << "EUQ_Turb_SA()" << endl;
...
public:   // member vars
	...
	double *kine;                 ///<kinetic energy based on SA muT
...
public:   // member functions
	...
	void perturbStress()
	{
		...
		for (int icv=0; icv<ncv; icv++)
		{
			...
			// from equation k=muT_SA * vortMag / (sqrt(0.3) * rho)
			// make it so user specifies kine equation
			kine[icv] = muT[icv] * vortMag[icv] * 1.82574 / rho[icv];
