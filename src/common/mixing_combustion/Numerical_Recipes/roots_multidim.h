#ifndef ROOTS_MULTIDIM_H
#define ROOTS_MULTIDIM_H
...
VecDoub_O &x, Doub &f, const Doub stpmax, Bool &check, T &func) {
	...
	test=0.0;
	for (i=0;i<n;i++) {
		temp=abs(p[i])/MAX_NR(abs(xold[i]),1.0);
...
void newt(VecDoub_IO &x, Bool &check, T &vecfunc, Int MAXITS=200) {
	//const Int MAXITS=200;
	//const Doub TOLF=1.0e-8,TOLMIN=1.0e-12,STPMX=100.0;
        const Doub TOLF=1.0e-5,TOLMIN=1.0e-7,STPMX=1.0;
	...
	stpmax=STPMX*MAX_NR(sqrt(sum),Doub(n));
	for (its=0;its<MAXITS;its++) {
		...
		if (check) {
			...
			den=MAX_NR(f,0.5*n);
			for (i=0;i<n;i++) {
				temp=abs(g[i])*MAX_NR(abs(x[i]),1.0)/den;
				if (temp > test) test=temp;
			}
			check=(test < TOLMIN);
			return;
		...
		test=0.0;
		for (i=0;i<n;i++) {
			temp=(abs(x[i]-xold[i]))/MAX_NR(abs(x[i]),1.0);
			if (temp > test) test=temp;
		}
		if (test < TOLX)
			return;
