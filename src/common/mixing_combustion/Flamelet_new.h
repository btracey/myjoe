...
private:
  ...
  double          *pdf,*pdf_lin;                   ///< Beta PDF used for convolution parametrized by a mean and variance.
  double          *z_inte, (*f_interp)[2];
...
public:
  ...
  ~Flamelet()
  {
    ...
    if (pdf_lin != NULL) delete [] pdf_lin; pdf_lin = NULL;
    if (z_inte != NULL) delete [] z_inte; z_inte = NULL;
    if (f_interp != NULL) delete [] f_interp; f_interp = NULL;
                    ...
                    const string DefinitionProg = "USUAL", const double Zst = 1.0)
  {
    ...
    pdf_lin = new double[nPoints];
    z_inte = new double[nPoints+1];
    f_interp = new double[nPoints][2];
...
//      }
  ...
  void ComputeBetaPDF(double mean, double var)
  {
    ...
    if ((mean != BetaMean) || (var != BetaVar))
    {
      ...
      ComputeBetaPDF_lin(mean, var);

      // check PDF Norm
      double integral = 0.0;
      for  (int k=0; k<nPoints; k++)
        integral += pdf[k];
      if (fabs(1.0-integral)>1.e-4) {
  ...
  void ComputeBetaPDF_lin(double mean , double var) {
    ...
    for  (int k=0; k<nPoints; k++) {
       z1 = z_inte[k];
       z2 = z_inte[k+1];
       pdf_lin[k] = betai_monomial(a,b,z2,1)-betai_monomial(a,b,z1,1);
       moy += pdf_lin[k];
       Z2_moy += betai_monomial(a,b,z2,2)-betai_monomial(a,b,z1,2);
       if (pdf_lin[k] != pdf_lin[k]) {
         cout << "Nan in pdf_lin  at k = " << k << ", " << betai_monomial(a,b,z2,1) << " " << betai_monomial(a,b,z1,1) << endl;
         cout << "  < parameters : " << a << " "<< b << " " << z2 << " " << z1 <<endl;
         throw(-1);
       }
    }
    double my_var = Z2_moy - moy*moy;

    if (fabs(moy-mean)/mean>1.e-4) {
      cerr << "Beta PDF new ("<<mean<<" , "<<var<<") not well discretized, mean: " << moy << " vs " << mean << endl;
      throw(-1);
    }

    if (fabs(my_var-var)/var>1.e-4) {
      cerr << "Beta PDF new ("<<mean<<" , "<<var<<") not well discretized, var: " << my_var << " vs " << var << endl;
      throw(-1);
    }
  ...
  void IntegrateBetaPiecewiseLinear(int iZ, int index) {
   // f = a*Z+b
   double a;
   double b;
   // k = 0
   int k = 0;
   a = (myData[k+1][index]-myData[k][index])/(myData[k+1][iZ]-myData[k][iZ]);
   b = myData[k][index] - a*myData[k][iZ];
   f_interp[k][0] = b;
   f_interp[k][1] = a;

   // centered for k in [1:nPoints-2]
   for (k=1; k<nPoints-1; k++) {
      a = (myData[k+1][index]-myData[k-1][index])/(myData[k+1][iZ]-myData[k-1][iZ]);
      b = myData[k][index] - a*myData[k][iZ];
      f_interp[k][0] = b;
      f_interp[k][1] = a;
   }
   ...
   // k = nPoints - 1 
   k = nPoints-1;
   a = (myData[k][index]-myData[k-1][index])/(myData[k][iZ]-myData[k-1][iZ]);
   b = myData[k][index] - a*myData[k][iZ];
   f_interp[k][0] = b;
   f_interp[k][1] = a;
  ...
  void IntegrateBetaPiecewiseLinear_inv(int iZ, int index) {
   // f = a*Z+b
   double a;
   double b;
   // k = 0
   int k = 0;
   a = (1.0/myData[k+1][index]-1.0/myData[k][index])/(myData[k+1][iZ]-myData[k][iZ]);
   b = 1.0/myData[k][index] - a*myData[k][iZ];
   f_interp[k][0] = b;
   f_interp[k][1] = a;

   // centered for k in [1:nPoints-2]
   for (k=1; k<nPoints-1; k++) {
      a = (1.0/myData[k+1][index]-1.0/myData[k-1][index])/(myData[k+1][iZ]-myData[k-1][iZ]);
      b = 1.0/myData[k][index] - a*myData[k][iZ];
      f_interp[k][0] = b;
      f_interp[k][1] = a;
   }
   ...
   // k = nPoints - 1 
   k = nPoints-1;
   a = (1.0/myData[k][index]-1.0/myData[k-1][index])/(myData[k][iZ]-myData[k-1][iZ]);
   b = 1.0/myData[k][index] - a*myData[k][iZ];
   f_interp[k][0] = b;
   f_interp[k][1] = a;
  ...
  /** \brief Convolute the flamelet with a Beta-PDF given PDF parameters mean and variance for given variable */
  double FlameletConvoluteWithPDF_new(double mean, double var, string variable)
  {
    ...
    double value = 0.0;
    if ((variable == "density") || (variable == "rho0")) { IntegrateBetaPiecewiseLinear_inv(J2,J); }
    else { IntegrateBetaPiecewiseLinear(J2,J); }

    for (int k=0; k<nPoints; k++) {
       value +=   pdf[k]    * f_interp[k][0]
                + pdf_lin[k]* f_interp[k][1];
    }
  
    if ((variable == "density") || (variable == "rho0")) { return 1.0 / value; }
    else { return value; }
  }
