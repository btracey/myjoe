 ...
 * coordinates using cubic interpolation.
 * 
...
#ifndef CHEMISTRYTABLECARTESIANCUBIC_H
#define CHEMISTRYTABLECARTESIANCUBIC_H
...
public:  
  ChemtableCartesianCubic()
  {
    if (mpi_rank == 0)
      cout << "ChemtableCartesianCubic()" << endl;
    ...
    interp = new InterpolationIndex(3,4);
  }
  ...
  // Copied and adapted from CDP chemtable_m.f90
  {
    ...
    // initialize quadratic extrapolation weights
    // x1 dimension: quadratic through 1,2,3
    xi=x1[0], x0=x1[1], xp1=x1[2], xp2=x1[3];
    wextrap_1[0][0] =  (xi-xp2) * (xi-xp1) / (x0 -xp2) / (x0-xp1);
    wextrap_1[1][0] = -(xi-xp2) * (xi-x0)  / (xp1-xp2) / (x0-xp1);
    wextrap_1[2][0] =  (xi-xp1) * (xi-x0)  / (xp1-xp2) / (x0-xp2);
    // x1 dimension: quadratic through n-2,n-1,n
    xi=x1[n1+1], x0=x1[n1-2], xp1=x1[n1-1], xp2=x1[n1];
    wextrap_n[0][0] =  (xi-xp2) * (xi-xp1) / (x0 -xp2) / (x0-xp1);
    wextrap_n[1][0] = -(xi-xp2) * (xi-x0)  / (xp1-xp2) / (x0-xp1);
    wextrap_n[2][0] =  (xi-xp1) * (xi-x0)  / (xp1-xp2) / (x0-xp2);

    // x2 dimension: quadratic through 1,2,3
    xi=x2[0], x0=x2[1], xp1=x2[2], xp2=x2[3];
    wextrap_1[0][1] =  (xi-xp2) * (xi-xp1) / (x0 -xp2) / (x0-xp1);
    wextrap_1[1][1] = -(xi-xp2) * (xi-x0)  / (xp1-xp2) / (x0-xp1);
    wextrap_1[2][1] =  (xi-xp1) * (xi-x0)  / (xp1-xp2) / (x0-xp2);
    // x2 dimension: quadratic through n-2,n-1,n
    xi=x2[n2+1], x0=x2[n2-2], xp1=x2[n2-1], xp2=x2[n2];
    wextrap_n[0][1] =  (xi-xp2) * (xi-xp1) / (x0 -xp2) / (x0-xp1);
    wextrap_n[1][1] = -(xi-xp2) * (xi-x0)  / (xp1-xp2) / (x0-xp1);
    wextrap_n[2][1] =  (xi-xp1) * (xi-x0)  / (xp1-xp2) / (x0-xp2);

    // x3 dimension: quadratic through 1,2,3
    xi=x3[0], x0=x3[1], xp1=x3[2], xp2=x3[3];
    wextrap_1[0][2] =  (xi-xp2) * (xi-xp1) / (x0 -xp2) / (x0-xp1);
    wextrap_1[1][2] = -(xi-xp2) * (xi-x0)  / (xp1-xp2) / (x0-xp1);
    wextrap_1[2][2] =  (xi-xp1) * (xi-x0)  / (xp1-xp2) / (x0-xp2);
    // x3 dimension: quadratic through n-2,n-1,n
    xi=x3[n3+1], x0=x3[n3-2], xp1=x3[n3-1], xp2=x3[n3];
    wextrap_n[0][2] =  (xi-xp2) * (xi-xp1) / (x0 -xp2) / (x0-xp1);
    wextrap_n[1][2] = -(xi-xp2) * (xi-x0)  / (xp1-xp2) / (x0-xp1);
    wextrap_n[2][2] =  (xi-xp1) * (xi-x0)  / (xp1-xp2) / (x0-xp2);
    ...
    for (int j=0; j<=n2+1; j++)
      for (int k=0; k<=n3+1; k++)
        ...
        {
          Data[0][j][k][l]    = wextrap_1[0][0] * Data[1]   [j][k][l] +
                                wextrap_1[1][0] * Data[2]   [j][k][l] +
                                wextrap_1[2][0] * Data[3]   [j][k][l];
          Data[n1+1][j][k][l] = wextrap_n[0][0] * Data[n1-2][j][k][l] +
                                wextrap_n[1][0] * Data[n1-1][j][k][l] +
                                wextrap_n[2][0] * Data[n1]  [j][k][l];
        }                                                                    
    // x2 dimension
    for (int i=0; i<=n1+1; i++)
      for (int k=0; k<=n3+1; k++)
        for (int l=0; l<nvar; l++)
        {
          Data[i][0][k][l]    = wextrap_1[0][1] * Data[i][1]   [k][l] + 
                                wextrap_1[1][1] * Data[i][2]   [k][l] +
                                wextrap_1[2][1] * Data[i][3]   [k][l];
          Data[i][n2+1][k][l] = wextrap_n[0][1] * Data[i][n2-2][k][l] +
                                wextrap_n[1][1] * Data[i][n2-1][k][l] +
                                wextrap_n[2][1] * Data[i][n2]  [k][l];
        }     
    // x3 dimension
    for (int i=0; i<=n1+1; i++)
      for (int j=0; j<=n2+1; j++)
        for (int l=0; l<nvar; l++)
        {
          Data[i][j][0][l]    = wextrap_1[0][2] * Data[i][j][1]   [l] +
                                wextrap_1[1][2] * Data[i][j][2]   [l] +
                                wextrap_1[2][2] * Data[i][j][3]   [l];
          Data[i][j][n3+1][l] = wextrap_n[0][2] * Data[i][j][n3-2][l] +
                                wextrap_n[1][2] * Data[i][j][n3-1][l] +
                                wextrap_n[2][2] * Data[i][j][n3]  [l];
        }
  ...
  // Compute weights
  {
    double xm1, x0, xp1, xp2;
    
    // First direction
    BinarySearch(interp->index[0], x1, 1, n1, A1);
    
    xm1 = x1[interp->index[0] - 1];
    x0  = x1[interp->index[0]];
    xp1 = x1[interp->index[0] + 1];
    xp2 = x1[interp->index[0] + 2];
    
    Compute_Weights(A1, xm1, x0, xp1, xp2, interp->weight[0]);
    
    // Second direction
    BinarySearch(interp->index[1], x2, 1, n2, A2);
    
    xm1 = x2[interp->index[1] - 1];
    x0  = x2[interp->index[1]];
    xp1 = x2[interp->index[1] + 1];
    xp2 = x2[interp->index[1] + 2];
    
    Compute_Weights(A2, xm1, x0, xp1, xp2, interp->weight[1]);
    
    // Third direction
    BinarySearch(interp->index[2], x3, 1, n3, A3);
    
    xm1 = x3[interp->index[2] - 1];
    x0  = x3[interp->index[2]];
    xp1 = x3[interp->index[2] + 1];
    xp2 = x3[interp->index[2] + 2];
    
    Compute_Weights(A3, xm1, x0, xp1, xp2, interp->weight[2]);
   ...
   *  Use cubic polynomial going through points f(i-1:i+2)
   *  Weights are Lagrange polynomials evaluated at x l_j(x)=Product_(k=0, k!=j)^(k=3) (x-x_k)/(x_j-x_k)
   */
  void Compute_Weights(double x, double xm1, double x0, double xp1, double xp2, double *wi)
  {
    double xxm1, xx0, xxp1, xxp2;
    double xm1x0, xm1xp1, xm1xp2, x0xp1, x0xp2, xp1xp2;
    ...
    // weight for f(i-1)
    wi[0] =   xxp2 * xxp1 * xx0  / (xm1xp2 * xm1xp1 * xm1x0);
    
    // weight for f(i)
    wi[1] = - xxp2 * xxp1 * xxm1 / (x0xp2  * x0xp1  * xm1x0);

    // weight for f(i+1)
    wi[2] =   xxp2 * xx0  * xxm1 / (xp1xp2 * x0xp1  * xm1xp1);
    
    // weight for f(i+2)
    wi[3] = - xxp1 * xx0  * xxm1 / (xp1xp2 * x0xp2  * xm1xp2);
   ...
   *
   *  Interpolation is cubic. Any variable contained in table can be looked up. Indices and weights must be
  ...
  double Lookup(double A1, double A2, double A3, string tag)
  {
    ...
    // Use Lagrange polynomials: y(x) = Sum_j=0^j=3 y_j * l_j(x)  where l_j(x) are the weights
    val = 0.0;    
    for (int k = 0; k < 4; k++)
    {
      g[k] = 0.0;
      for (int j = 0; j < 4; j++)
      {
        f[j] = 0.0;
        for (int i = 0; i < 4; i++)
          f[j] += interp->weight[0][i] * Data[interp->index[0]-1+i][interp->index[1]-1+j][interp->index[2]-1+k][ll];
   ...
   *
   *  Interpolation is cubic. All coefficients contained in table are looked up. Indices and weights are
   ...
   *
   *  Interpolation is cubic. Selected coefficients contained in table are looked up. Indices and weights are
   *  first computed using Chemtable::SetCoordinates. Returns the interpolated value of the coefficients.
   *  \param[in]  A1, A2, A3                   Coordinates.
