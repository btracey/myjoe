 ...
 * 
 *  This version of the chemistry table is based on a 3D structured adaptive tabulation,
 *  i.e., the 3rd dimension is a function of the first 2.
...
#ifndef CHEMISTRYTABLEADAPTIVELINEAR_H
#define CHEMISTRYTABLEADAPTIVELINEAR_H
...
private:
  ...
  double             *x1, *x2;                         ///< Axis coordinates of the table (Zmean, Zvar), just vectors
  double             ***x3;                            ///< Axis coordinates of the table (chi/progress variable), function of first 2 coordinates
...
public:  
  ChemtableAdaptiveLinear()
  {
    if (mpi_rank == 0)
      cout << "ChemtableAdaptiveLinear()" << endl;
  ...
  double GetChemtableCoordinate3(int i1, int i2, int i3) {return x3[i1][i2][i3];}
  ...
  int    GetChemtableIndex3(double A1, double A2, double A3) {SetCoordinates(A1, A2, A3); return interp->index[2];}
  ...
  void Load(string tablename)
  {
    ...
    getMem3D(&x3, 0, n1+1, 0, n2+1, 0, n3+1, "Chemtable::Load x3", true);  // padding: add 1 point at beginning and end of table for interpolation
    ...
    // the third dimension is a function of the first and second
    for (i=1; i<n1+1; i++)
      for (j=1; j<n2+1; j++)
        for (k=1; k<n3+1; k++)
        {
          dum = fread(&x3[i][j][k], sizeof(double), 1, inFp);
          if (dum != 1)
          {
            cerr << "### Error reading file: x3 -> " << i << " " << j << " " << k << " ###" << endl;
            throw(-1);
          }
        }
  ...
  void Unload()
  {
    ...
    if (x3 != NULL)       {freeMem3D(x3, 0, n1+1, 0, n2+1, 0, n3+1);                x3 = NULL;}
  ...
  // Copied and adapted from CDP chemtable_m.f90
  {
    ...
    for (int i=1; i<n1+1; i++)
      ...
      {
        x3[i][j][0]    = 2.0 * x3[i][j][1]  - x3[i][j][2];
        x3[i][j][n3+1] = 2.0 * x3[i][j][n3] - x3[i][j][n3-1];
      }
  ...
  // Compute weights
  {
    ...
    // Third direction (depends on first 2 directions!!!)
    double P0, P1, f[2];
    ...
    for (int j = 0; j < 2; j++)
    {
      f[j] = 0.0;
      for (int i = 0; i < 2; i++)
        f[j] += interp->weight[0][i] * x3[interp->index[0]+i][interp->index[1]+j][1];
      P0 += interp->weight[1][j] * f[j];
    }
    
    if (A3 <= P0)    // i.e., A3 < x3_min  =>  clipping
    {
      interp->index[2] = 1;
      interp->weight[2][0] = 1.0;
    }
    else
    {
      for (int k=2; k<=n3; k++)
      {
        ...
        for (int j = 0; j < 2; j++)
        {
          f[j] = 0.0;
          for (int i = 0; i < 2; i++)
            f[j] += interp->weight[0][i] * x3[interp->index[0]+i][interp->index[1]+j][k];
          P1 += interp->weight[1][j] * f[j];
        }
        if (A3 <= P1)
        {
          interp->index[2] = k - 1;
          if (P1 == P0)
            ...
            interp->weight[2][0] = (P1 - A3) / (P1 - P0);
          break;
        }
        P0 = P1;
        if (k == n3)    // i.e., A3 > x3_max  =>  clipping
        {
          interp->index[2] = n3 - 1;
          interp->weight[2][0] = 0.0;
        }
      }      
    }
   ...
   *  \return    Interpolated value of variable \p tag.
   */
  double LookupGradCmeanSource(double A1, double A2, double A3)
  {
    int ll;
    double val, g[2], f[2];
    ...
    // Determine the index of the source term
    string tag = "SRC_PROG";
    itTp = myTableVar.find(tag);
    if (itTp == myTableVar.end())
    {
    ...
    double DeltaCmeanSource = g[1] - g[0];
    
    // Determine the index of the progress variable
    tag = "PROG";
    itTp = myTableVar.find(tag);
    if (itTp == myTableVar.end())
    {
    ...
    double DeltaCmean = g[1] - g[0];
//    cout << DeltaCmeanSource << "    " << DeltaCmean << "    " << DeltaCmeanSource / DeltaCmean << endl;
    
    if (DeltaCmean == 0.0)
      return 0.0;
    else
      return DeltaCmeanSource / DeltaCmean;
  }
